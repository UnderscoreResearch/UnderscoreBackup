...
class BackupContentsAccessImplTest {
    ...
    public void test() throws IOException {
        ...
        fileSystemAccess = Mockito.mock(FileSystemAccess.class);

        Mockito.when(metadataRepository.lastDirectory(""))
                .thenReturn(Sets.newTreeSet(Lists.newArrayList("/test/set1/", "/test/set2/", "/")));
        NavigableMap<Long, NavigableSet<String>> treeMap = new TreeMap<>();
        treeMap.put(2L, Sets.newTreeSet(Lists.newArrayList("/test/set1/", "/")));
        treeMap.put(3L, Sets.newTreeSet(Lists.newArrayList("/test/set1/", "/test/set2/", "/")));
        Mockito.when(metadataRepository.directory("")).thenReturn(treeMap);


        Mockito.when(metadataRepository.lastDirectory("/"))
                .thenReturn(Sets.newTreeSet(Lists.newArrayList("dir2/")));
        treeMap = new TreeMap<>();
        treeMap.put(2L, Sets.newTreeSet(Lists.newArrayList("dir1/", "dir2/")));
        treeMap.put(3L, Sets.newTreeSet(Lists.newArrayList("dir2/")));
        Mockito.when(metadataRepository.directory("/")).thenReturn(treeMap);

        treeMap = new TreeMap<>();
        treeMap.put(2L, Sets.newTreeSet(Lists.newArrayList("file1", "dir/")));
        treeMap.put(3L, Sets.newTreeSet(Lists.newArrayList("file1", "file2", "dir/")));
        Mockito.when(metadataRepository.directory("/test/set1/")).thenReturn(treeMap);
        Mockito.when(metadataRepository.lastDirectory("/test/set1/"))
                .thenReturn(Sets.newTreeSet(Lists.newArrayList("file1", "file2", "dir/")));
        ...
        backupContentsAccess = new BackupContentsAccessImpl(metadataRepository, fileSystemAccess, null);
        backupContentsAccessCurrent = new BackupContentsAccessImpl(metadataRepository, fileSystemAccess, Instant.now().toEpochMilli());
        backupContentsAccessEarly = new BackupContentsAccessImpl(metadataRepository, fileSystemAccess, 2L);
        backupContentsAccessTooEarly = new BackupContentsAccessImpl(metadataRepository, fileSystemAccess, 1L);
    }
    ...
    public void testEarly() throws IOException {
        ...
        assertThat(backupContentsAccessEarly.directoryFiles("/test/"), Is.is(newFileSet(file("/test/set1/"))));
    ...
    public void testCurrent() throws IOException {
        ...
        assertThat(backupContentsAccessCurrent.directoryFiles("/test/"), Is.is(newFileSet(file("/test/set1/"), file("/test/set2/"))));
        ...
        assertThat(backupContentsAccess.directoryFiles("/test"), Is.is(newFileSet(file("/test/set1/"), file("/test/set2/"))));
