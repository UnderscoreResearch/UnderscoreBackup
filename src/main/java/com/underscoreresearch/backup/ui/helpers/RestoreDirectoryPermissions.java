...
/**
 * Manages the restoration of directory permissions during file restoration.
 * This class tracks directories that need their permissions restored after
 * all files have been restored to them.
 */
@Slf4j
public class RestoreDirectoryPermissions implements Closeable {
    ...
    /**
     * Creates a new RestoreDirectoryPermissions instance.
     *
     * @param repository The metadata repository
     * @param scheduler The download scheduler
     * @param fileSystemAccess The file system access implementation
     * @param skipPermissions If true restore any permissions
     * @throws IOException If there is an error initializing the permissions tracker
     */
    public RestoreDirectoryPermissions(MetadataRepository repository, DownloadScheduler scheduler,
                                       FileSystemAccess fileSystemAccess, boolean skipPermissions) throws IOException {
        if (!skipPermissions) {
    ...
    /**
     * Gets or initializes the large map for storing pending directory permissions.
     * This method lazily initializes a persistent map for tracking directories
     * that need their permissions restored when the in-memory cache is not sufficient.
     *
     * @return A closeable sorted map for storing pending directory information
     */
    ...
    /**
     * Creates a directory with the appropriate permissions during restore.
     *
     * @param destinationFile The destination file object
     * @param path The normalized path of the directory
    ...
    /**
     * Marks a file as completed in the restore process.
     * This may trigger directory permission restoration if all files in a directory are complete.
     *
     * @param path The path of the completed file
     */
    ...
    /**
     * Recursively processes parent directories when a directory is completed.
     * This method is called when all files in a directory have been restored,
     * allowing parent directories to be marked as complete if appropriate.
     *
     * @param parentPath The parent path to process
     */
    ...
    /**
     * Inner class representing a directory with pending permission restoration.
     * Tracks the permission to apply, the file path, and which files are still pending
     * before the directory permissions can be restored.
     */
    @Data
    @AllArgsConstructor
    @NoArgsConstructor
